...
export default ({ mode }) => {
  ...
  const useLocalEnv = (String(env.VITE_USE_LOCAL || '').toLowerCase() === '1') || (String(env.VITE_USE_LOCAL || '').toLowerCase() === 'true')
  ...
  const proxyTarget = mode === 'production'
    ? prodApi
    : (useLocalEnv ? (localApi || 'http://localhost:5000') : (localApi || prodApi))
        ...
        '/api': {
          ...
          configure: (proxy) => {
            ...
            if (mode !== 'production' && !useLocalEnv && !localApi) {
              ...
              console.log(`[vite] VITE_USE_LOCAL enabled, defaulting local proxy target to http://localhost:5000`)
            }
